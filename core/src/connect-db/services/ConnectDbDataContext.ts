...
import { MongooseDataContext } from "../../mongoose/services/MongooseDataContext";
...
import { WebhookSubscription, WebhookEventRecord } from "../../../../shared/models/webhooks/WebhookTypes";
...
export class ConnectDbDataContext extends MongooseDataContext {
   ...
   *
   * WARNING: this repository ultimately provides access to the same data as {@link users},
   ...
   */
  readonly unregisteredUsers: MongooseRepository<UnregisteredUser>;

  readonly users: MongooseRepository<IUser>;
  ...
  constructor() {
    ...
    this.users = this.createRepository<IUser>("UserDetail", schemas.UserSchema);
    this.unregisteredUsers = this.createRepository<UnregisteredUser>("UnregisteredUser", schemas.UnregisteredUserSchema, this.users.collectionName);
