...
import { useGenerateBscMacaroon, useInitializeNode } from "@lightning/hooks/mutations";
...
export const NetworkSync = observer(({ next, setup, setSetup }: Props) => {
  ...
  const { generateBscMacaroon, bscMac, bscLoading, bscMacError } = useGenerateBscMacaroon();
  ...
  // handle comp mount
  useEffect(() => {
    if ((!setup.password || setup.seed.length === 0) && !nodeDoc) router.replace("/infrastructure/lightning-connect/setup");
  }, []);
  ...
  const addVaultMacToStorage = async () => {
    try {
      await getMacaroon(setup.password, nodeDoc?.data?.nodeId, true);
      return true;
    } catch (e) {
      return false;
    }
  ...
  const initializeNode = async () => {
    if (triggerCreateBSCMacaroonAdminMacaroon) return;
    ...
    if (!success) {
      ...
      if (!setup.password) return router.replace({ pathname: "/infrastructure/lightning-connect", query: { setup: SetupSteps.SetPassword } });
  ...
  return (
    <StyledNetworkSync>
      ...
      {loaded ? <Title>Finished setting up Lightning Connect!</Title> : <LoadingBar height={barHeight}>{bars.map((nub) => nub)}</LoadingBar>}
      <StepButton id="btnLncSyncing" margin={"3rem auto 3.25rem"} width={"16rem"} disabled={!loaded} onClick={() => router.replace({ pathname: "/infrastructure/lightning-connect" })}>
        Finish
      </StepButton>
    </StyledNetworkSync>
  );
