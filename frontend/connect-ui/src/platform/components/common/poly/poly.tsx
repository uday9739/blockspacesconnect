...
export const Poly: TextComponent = React.forwardRef(<C extends React.ElementType = "span">({ as, color, className, children }: TextProps<C>, ref?: PolymorphicRef<C>) => {
  const Component = as || "span";

  const style = color ? { style: { color } } : {};

  return (
    <Component { ...style } ref={ ref } className={ className }>
      {children}
    </Component>
  );
});
