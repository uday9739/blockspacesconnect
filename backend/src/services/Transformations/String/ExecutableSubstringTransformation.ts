...
export default class ExecutableSubstringTransformation extends ExecutableBaseTransformation {
    ...
    protected validate():{isValid:Boolean;response:TExecutableConnectionResponse} {
        ...
        switch (true) {
            case (Object.keys(this.parameters).length < 2):
              ...
              returnResponse.message="Substring: Need at least 2 parameter";
            ...
            case (typeof String(this.parameters.string) !== 'string'):
              isValid = false;
              returnResponse.responseCode="500";
              returnResponse.message="Substring: string must be a string";
              logger.error("validate()", { module: thisModule }, { context: this.parameters }, {error:returnResponse.message});
              break;
            case (isNaN(Number(this.parameters.startIndex))):
              isValid = false;
              returnResponse.responseCode="500";
              returnResponse.message="Substring: startIndex must be a number";
              logger.error("validate()", { module: thisModule }, { context: this.parameters }, {error:returnResponse.message});
              break;
            case (this.parameters.numChars && isNaN(Number(this.parameters.numChars))):
              isValid = false;
              returnResponse.responseCode="500";
              returnResponse.message="Substring: when passed, numChars must be a number";
              logger.error("validate()", { module: thisModule }, { context: this.parameters }, {error:returnResponse.message});
              break;
        };
    ...
    protected transform():TExecutableConnectionResponse {
        ...
        try {
            returnResponse = {
                ...
                message:"Success",
                data: {
                    "result":String(this.parameters.string).substring(Number(this.parameters.startIndex) || 0, this.parameters.numChars ? Number(this.parameters.numChars)+Number(this.parameters.startIndex) : String(this.parameters.string).length)
                }
            }
        } 
        catch (error:any) {
            returnResponse = {
                responseCode:"500",
                message:`Substring: Error joining strings[] with separator - ${error}`,
                data: this.parameters
            };
