...
import { safeStringify, getJsonValue, getType, getReactRoot, getDefaultCommandOptions, checkReactOptsIsValid, getReactNode } from './utils';
 ...
 */
export function react(subject, component, reactOpts = {}) {
  if (subject === null) {
      ...
      .then(
        ...
        (window) => {
          ...
          elements.forEach((elm) => {
            const node = elm.node;
            const isFragment = elm.isFragment;
            if (!node) {
              return;
            }
            if (isFragment) {
 ...
 * }
 */
export function getReact(subject, component, reactOpts = {}) {
  if (subject === null) {
    throw new Error(`Previous component found null.`);
  }
 ...
 * @param {*} propName
 */
export function getProps(subject, propName) {
  const reactNode = getReactNode(subject);
  cy.log(`Finding value for prop **${propName || 'all props'}**`);
  cy.log(
    `Prop value found **${
      propName ? safeStringify(getJsonValue(reactNode.props, propName)) : safeStringify(reactNode.props)
    }**`
  );
  const propValue = propName ? cy.wrap(getJsonValue(reactNode.props, propName)) : cy.wrap(reactNode.props);
  return propValue;
}
 ...
 * @param {*} subject
 */
export function getCurrentState(subject) {
  const reactNode = getReactNode(subject);
  cy.log(`Finding current state of the React component`);
  cy.log(
    `Current state found **${
      getType(reactNode.state) === 'object' ? safeStringify(reactNode.state) : reactNode.state
    }**`
  );
 ...
 * @param {number} pos
 */
export function nthNode(subject, pos) { return cy.wrap(subject).then((e) => e[pos]); }